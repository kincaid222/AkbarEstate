...
const handleImageSubmit = (e) => {
    ...
    if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
      ...
      Promise.all(promises)
        ...
        .catch((err) => {
          setImageUploadError("Image uploade failed (2 mb max per image)");
          setUploading(false);
        });
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.imageUrls.length < 1)
        return setError("You must uploade at least one image");
        ...
        <div className="flex flex-col flex-1 gap-4">
          ...
          {formData.imageUrls.length > 0 &&
            formData.imageUrls.map((url, index) => (
              <div key={url} className="flex justify- p-3 border items-center">
                <img
                  src={url}
                  alt="listing image"
                ...
                <button
                  ...
                  className="p3 text-red-700 rounded-lg uppercase
                   hover:opacity-95"
                >
                  Delete
                </button>
